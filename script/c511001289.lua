...
function c511001289.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,Card.IsSetCard,1,e:GetHandler(),0x209) 
		and e:GetHandler():GetFlagEffect(511001289)==0 end
	local g=Duel.SelectReleaseGroup(tp,Card.IsSetCard,1,1,e:GetHandler(),0x209)
...
function c511001289.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:GetFlagEffect(511001289)==0 then
		...
		c:RegisterEffect(e4)
		c:RegisterFlagEffect(511001289,RESET_EVENT+0x1fe0000+RESET_PHASE+RESET_END,0,1)
	end
end
