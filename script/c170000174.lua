...
function c170000174.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not e:GetHandler():IsReason(REASON_REPLACE) and e:GetHandler():GetCounter(0x97)>0 end
	e:GetHandler():RemoveCounter(tp,0x97,1,REASON_EFFECT)
	return true
end
function c170000174.filter(c,ct)
	return c:GetCounter(0x97)==ct and c:IsFaceup() and c:IsCode(170000175)
end
