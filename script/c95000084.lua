...
function c95000084.initial_effect(c)
	...
	e4:SetTarget(c95000084.aclimit2)
	c:RegisterEffect(e4)
	--cannot activate
	...
	e5:SetValue(c95000084.aclimit)
	c:RegisterEffect(e5)
	--~ Add Action Card
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(95000084,0))
	e6:SetType(EFFECT_TYPE_QUICK_O)
	e6:SetRange(LOCATION_SZONE)
	e6:SetCode(EVENT_FREE_CHAIN)
	e6:SetCondition(c95000084.condition)
	e6:SetTarget(c95000084.Acttarget)
	e6:SetOperation(c95000084.operation)
	c:RegisterEffect(e6)
	--Atk
	local e6=Effect.CreateEffect(c)
	...
	ef:SetCountLimit(1)
	ef:SetRange(0xff)
	ef:SetOperation(c95000084.Cheatercheck1)
	c:RegisterEffect(ef)
	-- Draw when removed
	...
	ef3:SetCondition(c95000084.descon)
	ef3:SetTarget(c95000084.drtarget)
	ef3:SetOperation(c95000084.drop)
	c:RegisterEffect(ef3)
end
function c95000084.Cheatercheck1(e,c)
	if Duel.GetMatchingGroupCount(c95000084.Fieldfilter,tp,0,LOCATION_DECK+LOCATION_HAND,nil)>1
	then
	local WIN_REASON_ACTION_FIELD=0x55
	Duel.Win(tp,WIN_REASON_ACTION_FIELD)
	end
	
	local sg=Duel.GetMatchingGroup(c95000084.Fieldfilter,tp,LOCATION_DECK+LOCATION_HAND,LOCATION_DECK+LOCATION_HAND,nil)
	Duel.Remove(sg,POS_FACEUP,REASON_EFFECT)
end

function c95000084.descon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsFaceup() and e:GetHandler():IsPreviousLocation(LOCATION_HAND)
end
function c95000084.drtarget(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return true end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c95000084.drop(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Draw(p,d,REASON_RULE)
end
function c95000084.Fieldfilter(c)
	return c:IsSetCard(0xac2)
end
function c95000084.ctcon2(e,re)
	return re:GetHandler()~=e:GetHandler()
end
function c95000084.aclimit(e,re,tp)
...
e:GetHandler():RegisterFlagEffect(95000084,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
end
...
function c95000084.condition(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(c95000084.cfilter,tp,LOCATION_SZONE+LOCATION_HAND,0,1,nil) and e:GetHandler():GetFlagEffect(95000084)==0
